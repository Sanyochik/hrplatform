   ...
   <div class="row">
    <div class="col-lg-5">
        ...
        $lasttriger = (new \yii\db\Query())
            ...
            ->join('left JOIN', 'seasons','questions.season = seasons.id')
            ->max('triger');
        $lasttriger+=2;
        for ($i=1; $i < $lasttriger; $i++) { 
            ...
            ->select(['id_about'])
            ->from('answers')
            ->where(['id_creator'=>Yii::$app->user->getId(),'activity'=>1])
            ->join('left JOIN', 'questions','answers.id_question = questions.id')
            ->join('left JOIN', 'seasons','questions.season = seasons.id')
            ->all();
            $left=array();
        ...
        $already = (new \yii\db\Query())
            ...
            ->GROUPBY(['id_about'])
            ->all();
        if(($tempo)&&(count($already)<10)){
            $maxtriger = (new \yii\db\Query())
            ->select(['triger'])
            ->from('answers')
            ->where(['id_about'=>$tempo[0]['id'],'skip'=>null,'activity'=>1])
            ->join('left JOIN', 'questions','answers.id_question = questions.id')
            ->join('left JOIN', 'seasons','questions.season = seasons.id')
            ->max('triger');
            if(!$maxtriger){
            ...
            ->select(['skip'])
            ->from('answers')
            ->where(['id_creator'=>Yii::$app->user->getId(),'skip'=>1,'activity'=>1])
            ->join('left JOIN', 'questions','answers.id_question = questions.id')
            ->join('left JOIN', 'seasons','questions.season = seasons.id')
            ->count();
            ...
            ->select(['id_about'])
            ->from('answers')
            ->where(['id_creator'=>Yii::$app->user->getId(),'skip'=>null,'activity'=>1])
            ->join('left JOIN', 'questions','answers.id_question = questions.id')
            ->join('left JOIN', 'seasons','questions.season = seasons.id')
            ->count();

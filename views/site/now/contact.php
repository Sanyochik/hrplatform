   ...
   <div class="row">
    <div class="col-lg-5">
        ...
        if(($tempo)&&($_SESSION['count']<10)){
            ...
            $countall=(new \yii\db\Query())
            ->select(['id','fi','photo'])
            ->from('svaz')
            ->where(['id_otv'=>Yii::$app->user->getId(),'type'=>[2]])
            ->join('INNER JOIN', 'svazusers','id_svaz = svazusers.id')
            ->count();
            $countofskip = (new \yii\db\Query())
            ->select(['skip'])
            ->from('answers')
            ->where(['id_creator'=>Yii::$app->user->getId(),'skip'=>1])
            ->count();
            $countanswered = (new \yii\db\Query())
            ->select(['id_about'])
            ->from('answers')
            ->where(['id_creator'=>Yii::$app->user->getId(),'skip'=>null])
            ->count();
            $countleft = $countall/2-$countofskip;
            ...
            if(floor($countleft) > 0){
                echo '<h2 style="width:800px;">Если вы затрудняетесь с оценкой этого сотрудника вы можете пропустить его и еще '.floor($countleft).' сотрудника</h2>';
                echo'<a href="skip?who='.$tempo[0]['id'].'"><div class="btn btn-warning">Пропустисть</div></a>';
            }
            ...
            foreach ($array as $key => $value) {
                if(($value['id'] == 13)||($value['id'] == 12)){
                    ...
                    $value['label']=str_replace('Иван Иванов', '<b>'.$tempo[0]['fi'].'</b>', $value['label']);
                    $value['label']=str_replace('Ивана Иванова', ' сотрудника <b>'.$tempo[0]['fi'].'</b>', $value['label']);
                    echo $form->field($model, 'id_question')->radioList(array(1 => 'Никогда', 2 => 'Редко', 3 => 'В половине случаев', 4 => 'Часто'), array('name'=>$value['id']))->label($value['label']);
                    echo '<div style="margin-top:-15px;"><label class="form-check-label"><input value="5" class="form-check-input" type="radio" name="'.$value["id"].'" required>   Всегда</label></div>';
                }
            }

...
<div class="svazusers-view">
    ...
    if($currentuser){
        ...
        ->select(['user.id','fi','photo'])
        ->from('answers')
        ->where(['id_about' => $_GET['id'],'skip'=>null])
        ->join('INNER JOIN', 'user','answers.id_creator = user.id')
        ->join('INNER JOIN', 'svazusers','user.email = svazusers.email')
        ->GROUPBY(['id_creator'])
        ->all();
        echo 'Данного пользователя знают: '.$knowcount.' пользователей <a href="view?id='.$_GET['id'].'&know=1" class="">Подробнее↓</a><br>';
        echo 'С данным пользователем работают '.$workcount.' пользователем <a href="view?id='.$_GET['id'].'&know=2" class="">Подробнее↓</a><br>';
        echo 'Этот пользователь работает с: '.$workwithcount.' пользователями <a href="view?id='.$_GET['id'].'&know=3" class="">Подробнее↓</a><br>';
        echo 'Этот пользователь знает: '.$knowwithcount.' пользователей <a href="view?id='.$_GET['id'].'&know=4" class="">Подробнее↓</a><br>';
        echo 'Этот пользователь сделал: '.count($couplecountresult).' взаимных связей <a href="view?id='.$_GET['id'].'&know=5" class="">Подробнее↓</a><br>';
        echo 'Этот пользователь не получил взаимные связи с : '.count($uncouplecountresult).' пользователем <a href="view?id='.$_GET['id'].'&know=6" class="">Подробнее↓</a><br>';
        echo 'Статистика по вопросам оценило '.count($whoanswer).' пользователей<a href="view?id='.$_GET['id'].'&know=7" class="">Подробнее ↓</a><br>';
        if(isset($_GET['know'])){
            if($_GET['know'] == 1){
                ...
                if($tempo){
                  ...
                  ->from('answers')
                  ->where(['id_about'=>$tempo[0]['id'],'skip'=>null])
                  ->all();
                }
              $countforanswers=1;
              ...
              foreach ($whoanswer as $key => $value) {
                  echo'
                    <div class="graphic">
                    <div class="question" style="background:url(/web/img/'.$value['photo'].');background-size: contain;background-repeat:no-repeat;width: 100%;height:50px;"></div><h3 style="margin-left:10%;" class="question">'.$value['fi'].'</h3>
                    ...
                    <div class="formax">';
                    $answersnow = (new \yii\db\Query())
                    ->select(['otvet','fi','photo'])
                    ->from('answers')
                    ->where(['id_about' => $_GET['id'],'id_creator'=>$value['id'],'skip'=>null])
                    ->join('INNER JOIN', 'user','answers.id_creator = user.id')
                    ->join('INNER JOIN', 'svazusers','user.email = svazusers.email')
                    ->all();
                    foreach ($answersnow as $key => $value) {
                        $width =0;
                        $width = $value['otvet']*100/5;
                        echo '<div class="progress-line1" style="width: '.$width.'%;background-color:'.$colors[$key].';">';
                        $questions[$key]['label'] = str_replace('Иван Иванов','<b>'.$tempo[0]['fi'].'</b>', $questions[$key]['label']);
                        $questions[$key]['label'] = str_replace('Ивана Иванова','сотрудник <b>'.$tempo[0]['fi'].'</b>', $questions[$key]['label']);
                        echo'
                        <p class="score">'.$value['otvet'].'</p>
                        </div>';
                        echo '<span class="hiddenprogress">'.$questions[$key]['label'].'</span>';
                    }
                    echo'
                    </div></div>';
              }
              echo '
              <div class="graphic">
              <h3 class="question">Среднее значение</h3>
              <p style="margin-left:-8%;margin-top:92px; margin-bottom:-400px;font-size:30px;"><b>н<br>и<br>к<br>о<br>г<br>д<br>а</b></p>
              <p style="margin-bottom:-370px;margin-top:92px;margin-left:105%; font-size:30px;"><b>в<br>с<br>е<br>г<br>д<br>а</b></p>
              <div class="formax">';
                for ($i=0; $i < $questioncount; $i++) {
                    ...
                    ->select(['id'])
                    ->from('answers')
                    ->where(['id_about'=>$tempo[0]['id'],'id_question'=>$countforanswers,'skip'=>null])
                    ->count();
                  ...
                  $questions[$i]['label'] = str_replace('Иван Иванов','<b>'.$tempo[0]['fi'].'</b>', $questions[$i]['label']);
                    ...
                    echo'
                  <p class="score">'.round($answersresult,1).'</p>
                  </div>';
